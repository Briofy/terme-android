...
import net.terme.offical.timeline.TimelineDecorator
...
class BaseAdapter(private val items: List<Int>) : RecyclerView.Adapter<BaseAdapter.BaseViewHolder>(),
    ...
    override fun getIndicatorStyle(position: Int): TermeTimelineView.IndicatorStyle{
        return if (position <= 1)
            TermeTimelineView.IndicatorStyle.Checked
        else TermeTimelineView.IndicatorStyle.Empty
    }

    override fun getLineStyle(position: Int): TermeTimelineView.LineStyle? {
        if (position > 1)
            return TermeTimelineView.LineStyle.Dashed
