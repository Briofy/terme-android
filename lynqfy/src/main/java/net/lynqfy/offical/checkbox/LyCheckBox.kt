...
import net.lynqfy.offical.utils.ScalingDrawable
import timber.log.Timber

class LyCheckBox : MaterialCheckBox {
    ...
    private fun initTheme(attr: AttributeSet?, defStyleAttr: Int = 0) {
        ...
        attr?.also {
            val attributes = context.obtainStyledAttributes(
                it, R.styleable.LyCheckBox, defStyleAttr,
                com.google.android.material.R.style.Widget_Material3_CompoundButton_CheckBox
            )
            activeTextColor =
                attributes.getColor(R.styleable.LyCheckBox_textColor, currentTextColor)
            tintColor = attributes.getColor(R.styleable.LyCheckBox_tintColor, Color.BLUE)
            val sizeValue = TypedValue()
            if (attributes.getValue(R.styleable.LyCheckBox_size, sizeValue)) {
                when (sizeValue.data) {
                    /*"small" */  1 -> scale(0.9f)
                    /*"medium"*/  2 -> scale(1f)
                    /*"large" */  3 -> scale(1.15f)
            ...
            var label = attributes.getString(R.styleable.LyCheckBox_label)
            if (label.isNullOrEmpty() || label.isEmpty()) {
                label = attributes.getString(R.styleable.LyCheckBox_name)
            }
            text = label

            val value = TypedValue()
            if (attributes.getValue(R.styleable.LyCheckBox_value, value)) {
                when (value.data) {
            ...
            isEnabled = !attributes.getBoolean(R.styleable.LyCheckBox_isDisabled, false)
            if (!isEnabled) {
                setTextColor(Color.GRAY)
            }

            isIndeterminate = attributes.getBoolean(R.styleable.LyCheckBox_isIndeterminate, false)
            isInvalid = attributes.getBoolean(R.styleable.LyCheckBox_isInvalid, false)
            isRequired = attributes.getBoolean(R.styleable.LyCheckBox_isRequired, false)
