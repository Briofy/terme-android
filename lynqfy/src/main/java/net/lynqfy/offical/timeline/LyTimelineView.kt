...
class LyTimelineView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.LyTimelineView,
            0, 0
        ).apply {
            try {
                viewType =
                    ViewType.values()[getInteger(
                        R.styleable.LyTimelineView_timeline_item_type,
                        viewType.ordinal
                    )]

                indicatorDrawable = getDrawable(R.styleable.LyTimelineView_indicator_drawable)

                indicatorSize = getDimensionPixelSize(
                    R.styleable.LyTimelineView_indicator_size,
                    indicatorSize.toInt()
                ).toFloat()

                checkedIndicatorSize = getDimensionPixelSize(
                    R.styleable.LyTimelineView_checked_indicator_size,
                    checkedIndicatorSize.toInt()
                ).toFloat()

                checkedIndicatorStrokeWidth = getDimensionPixelSize(
                    R.styleable.LyTimelineView_checked_indicator_stroke_width,
                    checkedIndicatorStrokeWidth.toInt()
                ).toFloat()

                indicatorYPosition = getFloat(
                    R.styleable.LyTimelineView_indicator_y_position,
                    indicatorYPosition
                ).coerceIn(0f, 1f)

                lineWidth = getDimensionPixelSize(
                    R.styleable.LyTimelineView_line_width,
                    lineWidth.toInt()
                ).toFloat()

                linePadding = getDimensionPixelSize(
                    R.styleable.LyTimelineView_line_padding,
                    linePadding.toInt()
                ).toFloat()

                lineDashLength = getDimensionPixelSize(
                    R.styleable.LyTimelineView_line_dash_length,
                    lineDashLength.toInt()
                ).toFloat()

                lineDashGap = getDimensionPixelSize(
                    R.styleable.LyTimelineView_line_dash_gap,
                    lineDashGap.toInt()
                ).toFloat()

                lineStyle =
                    LineStyle.values()[getInteger(R.styleable.LyTimelineView_line_style, lineStyle.ordinal)]

                lineColor = getColor(R.styleable.LyTimelineView_line_color, lineColor)

                indicatorColor = getColor(R.styleable.LyTimelineView_indicator_color, indicatorColor)
                indicatorStyle =
                    IndicatorStyle.values()[getInteger(R.styleable.LyTimelineView_indicator_style, indicatorStyle.ordinal)]
