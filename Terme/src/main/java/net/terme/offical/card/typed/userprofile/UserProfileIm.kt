...
import net.terme.offical.databinding.LyUserProfileUiBinding
    ...
    private fun initUserProfileTheme(attrs: AttributeSet?, defStyleAttr: Int = 0) {
        ...
        attrs?.also { att ->
            val attributes = parent.context.obtainStyledAttributes(
                att,
                R.styleable.TermeUserProfile,
                defStyleAttr,
                com.google.android.material.R.style.Widget_MaterialComponents_CardView
            )
            attributes.getString(R.styleable.TermeUserProfile_userName)?.let {
                ui.userName.text = it
            }
            attributes.getString(R.styleable.TermeUserProfile_userWork)?.let {
                ui.userWork.text = it
            }

            attributes.getDrawable(R.styleable.TermeUserProfile_userImage)?.let {
