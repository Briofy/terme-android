...
import net.terme.offical.databinding.LyEcommerceCardUiBinding
    ...
    private fun initEcommerceTheme(attrs: AttributeSet?, defStyleAttr: Int = 0) {
        ...
        attrs?.also { att ->
            val attributes = parent.context.obtainStyledAttributes(
                att,
                R.styleable.TermeEcommerce,
                defStyleAttr,
                com.google.android.material.R.style.Widget_MaterialComponents_CardView
            )
            attributes.getString(R.styleable.TermeEcommerce_ecommerceName)?.let {
                ui.name.text = it
            }
            attributes.getFloat(R.styleable.TermeEcommerce_ecommerceCast, 0f).let {
                ui.cost.text = buildString {
                    append("\$")
            ...
            attributes.getFloat(R.styleable.TermeEcommerce_ecommerceRating, 0f).let {
                ui.ratingValue.text = it.toString()
                ui.rating.rating = it
            }

            attributes.getDrawable(R.styleable.TermeEcommerce_ecommerceImage)?.let {
