...
import net.terme.offical.databinding.LyCryptoUiBinding
import net.terme.offical.databinding.LyItemUiBinding
import net.terme.offical.model.Badge
    ...
    private fun initCryptoTheme(attrs: AttributeSet?, defStyleAttr: Int = 0) {
        ...
        attrs?.also { att ->
            val attributes = parent.context.obtainStyledAttributes(
                att, R.styleable.TermeCrypto, defStyleAttr, MR.style.Widget_MaterialComponents_CardView
            )
            ...
            attributes.getString(R.styleable.TermeCrypto_helperText)?.let {
                ui.helper.text = it
            }
            attributes.recycle()
        }
